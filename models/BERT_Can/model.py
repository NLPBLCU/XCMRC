...
import model_doc_bert.ops as ops
...
class BaselineModel(object):
    def __init__(self, config, bert_config, vocab_size, emb):
        self.config = config
        ...
        # 候选项
        self.c = tf.placeholder('int32', [N, 10], name='c')

        self.y = tf.placeholder('int32', [N], name='y')

        self.bert_is_train = tf.placeholder('int32', [])
    ...
    def _build_forward(self):
        with tf.variable_scope("emb"):
            with tf.variable_scope("bert_emb"):
                ...
                (assignment_map, initialized_variable_names) = get_assignment_map_from_checkpoint(tvars, self.config.init_checkpoint)
        ...
        with tf.variable_scope("sentence_modeling"):
            ...
            self.attended_passage = self._bi_attention(self.config, self.is_train, self.doc_bert_repre, self.que_repre,
        ...
        with tf.variable_scope("summary"):
            ...
            self.doc_final_repre = tf.reduce_sum(self.attended_passage * rc_logits, 1)  # [N,dim]

        with tf.variable_scope('get_answer'):
            self.prediction = self._answer_layer(self.doc_final_repre, self.ans_word_emb)
    ...
    def _build_loss(self):
        ...
        mono_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=self.labels, logits=self.prediction))

        tf.add_to_collection('losses', mono_loss)
