...
def gen_examples(data, batch_size):
    ...
    for minibatch in minibatches:
        ...
        mb_x1, mb_mask1 = prepare_ans_data(mb_x1)
        mb_x2, mb_mask2 = prepare_data(mb_x2)
        mb_c, mb_maskc = prepare_ans_data(mb_c)
        yield {'ids': mb_x0, 'doc': mb_x1, 'que': mb_x2,
               'doc_mask': mb_mask1, "que_mask": mb_mask2,
               'candidates': mb_c, 'y': mb_y, "candidates_mask": mb_maskc}
...
def prepro_each(in_file, config, start_ratio=0, end_ratio=1):
    ...
    for idx, line in enumerate(lines[from_id:to_id]):
        ...
        if len(sample["passage"]) > config.max_p_num:
            continue
...
def vectorize(examples, word_dict, args,
    ...
    for idx, (m_id, d, t_q, a, c) in enumerate(zip(examples[0], examples[1], examples[2], examples[3], examples[4])):
        ...
        if args.no_passages:
            ...
            seq1 = []
            for i in range(args.max_p_num):
                if i < len(d):
                    para = [get_word_id(w, word_dict) for w in list(d[i])]
                    if len(para) < 512:
                        seq1.append(para)
                    else:
                        seq1 = []
        ...
        for w in t_q_words:
            seq2.append(get_word_id(w, word_dict))
        ...
        if seq1 and seq2:
            ...
            for j in range(args.candidate_nums):
                tmp = []
                for cid, a_c in enumerate(c[j]):
                    w_id = get_word_id(a_c, word_dict)
                    tmp.append(w_id)
                candidates.append(tmp)
            random.shuffle(candidates)
            ans = [get_word_id(char, word_dict) for char in a]
    ...
    if sort_by_len:
        ...
        sorted_index = ch_len_argsort(in_x1)
...
def vectorize_seq2seq(examples, word_dict, args,
              sort_by_len=True, verbose=True):
    """
        Vectorize `examples`.
        in_x1, in_x2: sequences for document and question respecitvely.
    ...
    in_x1 = []  # doc
    in_x2 = []  # que
    in_x3 = []  # source_question
    in_x4 = []  # target_question
    in_y = np.zeros((len(examples[0]), 1)).astype(np.float32)
    in_c = []
    n = 0
    #import pdb
    #pdb.set_trace()
    for idx, (m_id, d, t_q, s_q, a, c) in enumerate(zip(examples[0], examples[1], examples[2], examples[3], examples[4],examples[5])):
        d_words = d.split(' ')
        t_q_words = t_q.split(' ')
        s_q_words = s_q.split(' ')
        if args.cut_q:
        ...
        seq3 = [get_word_id(w, word_dict) for w in s_q_words]
        seq3.append(get_word_id('</s>', word_dict))

        seq4 = [get_word_id('<s>', word_dict)]
        for w in t_q_words:
            if w == "XXXX":
                seq4.append(get_word_id(a, word_dict))
            else:
                seq4.append(get_word_id(w, word_dict))
            seq2.append(get_word_id(w, word_dict))
        seq4.append(get_word_id('</s>', word_dict))

        assert a != 'unk'
        if (len(seq1) > 0) and (len(seq2) > 0) and (a in word_dict):
            if args.monolingual:
    ...
    if sort_by_len:
        ...
        in_c = [in_c[i] for i in sorted_index]
    return {'ids': in_x0, 'doc': in_x1, 'que': in_x2, 'target_sen': in_x4, 'source_sen': in_x3, 'candidates': in_c,
            'answer': in_y}
